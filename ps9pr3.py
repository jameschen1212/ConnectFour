...
class RandomPlayer(Player):
    def next_move(self, board):
        """that overrides (i.e., replaces) the next_move method that is inherited from Player. Rather than asking the user for the next move, this version of next_move should choose at random from the columns in the specified board that are not yet full, and return the index of that randomly selected column"""
        acol = []
        for i in range(board.width):
            if(board.can_add_to(i) == True):
                acol += [i]
        return random.choice(acol)
